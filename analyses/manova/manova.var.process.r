...
table(d$HER2)
d$HER2[d$HER2 %in% c('Borderline','Positive')] <- "BorderlineOrPositive"
...
d$DoseInterval[d$DoseInterval=='Q2.5wks'] <- NA
d$DoseInterval[d$DoseInterval=='Q3wks'] <- 3
table(d$DoseInterval)

table(d$FollowupStatus)
d$FollowupStatus[d$FollowupStatus %in% c("AWD","In Rx","LTFU")]=NA
table(d$FollowupStatus)
...
d$StagePre[d$StagePre%in%c("IIIB",'IIIC')] <- 3
...
d$StageFinal[d$StageFinal%in%c("IIIA","IIIB",'IIIC')] <- 3
d$StageFinal[d$StageFinal=='IV'] <- 4
table(d$StageFinal)
...
d$PercChangeTaxane = NULL
d$PercChangeRegimens = NULL
d$PercChangeNonTaxane = NULL


save.image(file="qc.clin.tri.manova.RDATA")
load("qc.clin.tri.manova.RDATA")
...
dim(bd)
write.csv(bd, file="C:/Users/Vestige/Google Drive/AlisonConsult/clinvitro/analyses/manova/AllVarProcessMan.csv")
